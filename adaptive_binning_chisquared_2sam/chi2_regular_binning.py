...
def chi2_regular_binning_wrapper(orig_name, dim_list, comp_file_list_list,single_no_bins_list,systematics_fraction):
	...
	for dim_index, dim_data in enumerate(dim_list):
		...
		for single_no_bins in single_no_bins_list:
			...
			for comp_file_0,comp_file_1 in comp_file_list:
				name = orig_name + "_" +str(dim_data) + "D_miranda_" + str(single_no_bins)+"_bins"
				...
				results_list = chi2_regular_binning(features_0,features_1,[single_no_bins],not counter,DEBUG)	
				score_list.append(results_list[0])
				counter+=1
